...
MySystem.DiagramRule=SC.Record.extend({suggestion:SC.Record.attr(String),comparison:SC.Record.attr(String),number:SC.Record.attr(Number),type:SC.Record.attr(String),hasLink:SC.Record.attr(Boolean),linkDirection:SC.Record.attr(String),otherNodeType:SC.Record.attr(String),energyType:SC.Record.attr(String),paletteItem:function(a){if(a=="node"){return null
}var c=SC.Query.local(MySystem.PaletteItem,"title = {title}",{title:a});var b=MySystem.store.find(c);
return b.objectAt(0)},energyTypeObject:function(){var b=this.get("energyType");if(!b||b=="any"){return null
}var c=SC.Query.local(MySystem.EnergyType,"label = {label}",{label:b});var a=MySystem.store.find(c);
return a.objectAt(0)},check:function(a){var b=this.matches(a);switch(this.get("comparison")){case"more than":return b>this.get("number");
...
}else{return b.get("uuid")==a.get("nodeType")}},checkLink:function(c,d,a){var b=this.energyTypeObject();
if(b&&b.get("uuid")!=c.get("energyType")){return false}return this.checkNode(d,c.get("startNode"))&&this.checkNode(a,c.get("endNode"))
...
}},this);return e}}}});sc_require("models/diagram_rule");MySystem.DiagramRule.FIXTURES=[{guid:"dr1",suggestion:"Diagram needs at least 3 nodes.",comparison:"more than",number:2,type:"node"},{guid:"dr2",suggestion:"At least one node must be Clay.",comparison:"more than",number:0,type:"Clay"},{guid:"dr3",suggestion:"At least one Clay node linked to a Hand node.",comparison:"more than",number:0,type:"Clay",hasLink:true,linkDirection:"-->",otherNodeType:"Hand"},{guid:"dr4",suggestion:"At least one Bulb node linked to a Hand node by thermal energy.",comparison:"more than",number:0,type:"Bulb",hasLink:true,linkDirection:"-->",otherNodeType:"Hand",energyType:"thermal energy"},{guid:"dr5",suggestion:"Diagram needs at least 2 links.",comparison:"more than",number:1,type:"node",hasLink:true,linkDirection:"---",otherNodeType:"node"}];
...
}});MySystem.InstructionView=SC.View.extend({childViews:"assignmentView checkButtonView".w(),backgroundColor:"#eeefff",canCollapse:YES,assignmentView:SC.LabelView.design({valueBinding:"MySystem.storyController.content",anchorLocation:SC.ANCHOR_TOP,layout:{top:5,right:5,bottom:5,left:5},tagName:"div",escapeHTML:NO,textAlign:SC.ALIGN_LEFT}),checkButtonView:SC.ButtonView.design({layout:{right:15,bottom:10,height:20,width:80},title:"Check",toolTip:"Check your diagram",action:"checkDiagramAgainstConstraints"})});
...
sc_require("views/user_story");MySystem.StoriesView=SC.SplitView.extend({layoutDirection:SC.LAYOUT_HORIZONTAL,topLeftView:MySystem.InstructionView,dividerView:SC.SplitDividerView,bottomRightView:MySystem.UserStoryView});
